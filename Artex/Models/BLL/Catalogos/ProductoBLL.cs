    ...
    public class ProductoBLL
    {
      ...
      */
        public List<piezasDTO> ObtenerPiezas(producto prodEntity, ArtexConnection db)
        {
            ...
            if (prodEntity != null)
            {
                var piezasAgregadas = db.piezas_producto.Where(m => m.ID_PRODUCTO == prodEntity.ID).Include(m => m.piezas_configurables);
                
                foreach (piezas_producto p in piezasAgregadas)
                {
                    piezasDTO dto = new piezasDTO();
                    dto.nombre = p.piezas_configurables.NOMBRE;
                    dto.id = p.ID_PIEZA;
                    dto.seleccionado = true;
                    dto.configurable = p.ES_CONFIGURABLE;

                    if (p.ES_CONFIGURABLE)
                    {
                        var comodinesAgregados = db.formulacion_comodin_v.Where(m => m.ID_PRODUCTO == prodEntity.ID && m.ID_PIEZA == p.ID_PIEZA);
                        dto.comodines = new List<ComodinesDTO>();

                        foreach (formulacion_comodin_v cmdn in comodinesAgregados)
                        {
                            ComodinesDTO cmdnDto = new ComodinesDTO();
                            cmdnDto.idFormulacionAtributo = cmdn.ID;
                            cmdnDto.idRaiz = cmdn.ID_COMODIN;
                            cmdnDto.nombre = cmdn.COMODIN;

                            dto.comodines.Add(cmdnDto);
                        }
